...
export default function UpdatePersonForm({
  ...
  person: z.infer<typeof peopleSelectSchema>;
}) {
  const [year, month, date] = person.birthday?.split("-");
  const birthday = new Date(+year, +month - 1, +date);
  const { id, ...values } = person;
  const form = useForm<z.infer<typeof peopleUpdateSchema>>({
    resolver: zodResolver(peopleUpdateSchema),
    defaultValues: { ...values, birthday },
  });

  return (
    <Form {...form}>
      <form
        onSubmit={form.handleSubmit(() => updatePerson(id, form.getValues()))}
